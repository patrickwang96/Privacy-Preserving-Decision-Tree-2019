...
void phase1(int num_trial, NetAdapter *net) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        for (int j = 0; j < num_trial; ++j) {
            CLOCK_START
            server_secure_multiplication(sel_mat.share[1], feature.share[1],
                                         tri.share[1],
                                         buf_E, buf_f,
                                         xsigma.share[1],
                                         0, net);
        ...
        printf("secure node selection (n=%d, d=%d, m=%d): %f ns, bytes: %f\n", n, param_nd[i][1], m,
...
//        }
        ...
        for (int j = 0; j < num_trial; ++j) {
            CLOCK_START
//            for (int k = 0; k < m; ++k)
            server_secure_node_evaluation(x, y, tri_z, tri_b, net);
            CLOCK_END
            time_total += ELAPSED;
        ...
        printf("secure node evaluation (n=%d, d=%d, m=%d): %f ns, bytes: %f\n", n, param_nd[i][1], m,
               time_total / num_trial / 2, send_mb + recv_mb); // count only one party
...
// secure node evaluation
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        for (int j = 0; j < num_trial; ++j) {
            CLOCK_START
//            for (int k = 0; k < m; ++k)
            server_node_eval_phase2(a, result, tri_b, net);
            CLOCK_END
            time_total += ELAPSED;
...
void phase3(int num_trial, NetAdapter *net) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        for (int j = 0; j < num_trial; ++j) {
            CLOCK_START
            server_secure_class_generation_path_cost(edges, leaf_value, interm_rlt, path_cost, d, net);
            CLOCK_END
            time_total += ELAPSED;
        ...
        printf("fig5 (n=%d, d=%d): %f ns, bytes: %f\n", n, d,
...
void phase4(int num_trial, NetAdapter *net) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        std::vector<mpz_class> path_cost(num_leaf);

        for (int j = 0; j < num_leaf; ++j) {
            leaf_value[i] = gmp_prn.get_z_bits(CONFIG_L);
            path_cost[i] = gmp_prn.get_z_bits(CONFIG_L);
        }
        ...
        printf("fig6 (n=%d, d=%d): %f ns, bytes: %f\n", n, d,
